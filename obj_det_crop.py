...
#from mrcnn import visualizeseg
# Import COCO config
...
MODEL_DIR = os.path.join(ROOT_DIR, "yseglogs")
SEG_DIR = os.path.join(ROOT_DIR, "randDetectCrop")
#SEG_DIR = os.path.abspath("D:/ML_data/notouchtest")
...
#IMAGE_DIR = os.path.join(ROOT_DIR, "4foods in 1")
IMAGE_DIR = os.path.abspath("C:/Users/hllun/ML_data/randData/randTest") #Need to change it down at collection creation at bottom
...
'''
# Load every images from the images folder
...
for i, image in enumerate(col):
    ...
    # Visualize results
    r = results[0]
    display_segs(image, r['rois'], r['masks'], r['class_ids'],
...
masked_image = image.astype(np.uint32).copy()
visualize.apply_mask(masked_image, r['masks'], [0.5,0.5,0.5])
'''
